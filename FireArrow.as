package  {
	...
	public class FireArrow extends MovieClip {
		public var speed:int = 30;//this is the speed of the FireArrow
		private var initialX:int;//the initial xlocation of the FireArrow
		public function FireArrow(playerX:int, playerY:int, playerDirection:String, playerSpeed:Number) {//when this funciton runs, you need to feed it 4 numbers for it to run
			if (playerDirection == "left") {//if the player is facing left then...
				speed = -speed+playerSpeed;//the speed of the FireArrow is 30 + the speed of the player (so he cant run after his FireArrow)
				x=playerX - 35;//the xlocation of the FireArrow is where the player is - 35 pixels
				this.scaleX *= -1;//flip te FireArrow so it faces left
		...
		public function everyFrame (e:Event):void {// stuff that should happen everyframe
			...
			if(speed > 0) {//if the FireArrow is going to the right then
